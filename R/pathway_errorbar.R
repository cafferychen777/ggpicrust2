...
#' @param abundance A data frame with row names representing pathways and column names representing samples. Each element represents the relative abundance of the corresponding pathway in the corresponding sample. Data frames may also provide a leading non-numeric feature ID column (for example \code{#NAME}, \code{feature}, or \code{pathway}); it is converted to row names before sample-count validation.
#' @param daa_results_df A data frame containing the results of the differential abundance analysis of the pathways, generated by the pathway_daa function. x_lab should be a column name of daa_results_df. Within the selected method and group pair, feature identifiers must be unique.
#' @param Group A data frame or a vector that assigns each sample to a group.
#' The groups are used to color the samples in the figure. Values must be
#' non-missing and non-empty for all abundance columns, and must include the
#' DAA result's \code{group1} and \code{group2} labels. A named vector is
#' aligned to abundance column names; otherwise the vector is interpreted in
#' abundance-column order.
...
#' @param p_values_threshold A numeric parameter specifying the threshold for statistical significance of differential abundance. Pathways with adjusted p-values below this threshold will be considered significant. Must be in the range (0, 1].
#' @param order A single character string controlling the ordering of the rows in the figure. The options are: "p_values" (order by p-values), "name" (order by pathway name), "group" (order by the group with the highest mean relative abundance), or "pathway_class" (order by the pathway category).
...
pathway_errorbar <-
  function(abundance,
    ...
    pvalue_format <- match.arg(pvalue_format,
                               c("numeric", "scientific", "smart",
                                 "stars_only", "combined"))
    validate_significance_inputs(0.05, pvalue_thresholds,
                                 pvalue_star_symbols,
                                 label_name = "pvalue_star_symbols")
    if (pvalue_colors && length(pvalue_thresholds) != 3) {
    ...
    # Subset to exactly the displayed DAA features. Do not use an
    # intersection here: if a significant DAA row cannot be found in the
    # abundance matrix, the abundance bars, p-value labels, and fold-change
    # panel would describe different feature sets.
    displayed_features <- as.character(daa_results_filtered_sub_df$feature)
    missing_features <- setdiff(displayed_features, rownames(relative_abundance_mat))
    if (length(missing_features) > 0) {
      stop(
        "pathway_errorbar() cannot plot DAA feature(s) missing from ",
        "abundance row names: ",
    ...
    sub_relative_abundance_mat <- relative_abundance_mat[displayed_features, , drop = FALSE]
    ...
    # Get groups for the current abundance columns. Check sample-name
    # coverage before checking values so an explicit `sample = NA` group
    # is reported as a missing group label, not as a missing sample mapping.
    missing_sample_names <- !colnames(sub_relative_abundance_mat) %in%
      names(sample_to_group_map)
    if (any(missing_sample_names)) {
      missing_samples <- colnames(sub_relative_abundance_mat)[missing_sample_names]
      stop(
    ...
    required_groups <- unique(c(
      as.character(daa_results_filtered_sub_df$group1),
      as.character(daa_results_filtered_sub_df$group2)
    ))
    correct_groups <- validate_group_vector_for_summary(
      correct_groups,
      context = "Group",
      sample_ids = colnames(sub_relative_abundance_mat),
    ...
    Group_levels <- unique(as.character(correct_groups))
    ...
    validate_choice(order, valid_orders, "order")
    ...
    validate_finite_numeric_values(
      daa_results_filtered_sub_df$log2_fold_change,
      "log2_fold_change",
      "daa_results_df",
      allow_na = FALSE
    )
