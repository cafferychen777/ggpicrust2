...
Stay up to date with the latest *ggpicrust2* developments by following me on Twitter: [![Follow CafferyYang on Twitter](https://img.shields.io/twitter/follow/CafferyYang?style=social)](https://twitter.com/CafferyYang)
...
The taxa contribution workflow connects PICRUSt2 per-sequence outputs with downstream pathway interpretation. Use `read_contrib_file()` for `pred_metagenome_contrib.tsv`, or `read_strat_file()` for `pred_metagenome_strat.tsv`, then aggregate to the taxonomic level you want to visualize.
